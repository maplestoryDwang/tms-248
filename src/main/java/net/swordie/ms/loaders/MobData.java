...
//        String[] wzDirs = new String[]{"/Mob.wz", "/Mob2.wz"};
        ...
        File questCountGroup = new File(ServerConstants.WZ_DIR + wzDirs[0] + "/QuestCountGroup_000.wz/");
        for (File dirFile : files) {
            if (dirFile.isDirectory()) {
                // QuestCountGroup
                if (dirFile.getName().contains("QuestCountGroup_000.wz")) {
                    continue;
                }
                Arrays.stream(dirFile.listFiles()).parallel().forEach(f->{
                    Document doc = XMLApi.getRoot(f);
