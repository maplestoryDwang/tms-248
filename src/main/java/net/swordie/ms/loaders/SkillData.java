    ...
    private static void loadSkillsFromWz() {
        ...
        for (String wz : wzs) {
            ...
            for (File subdir : subDirs) {
                if (subdir.getName().contains("MobSkill.wz")) {
                    continue;
                }
...
//        skill.setMasterLevel(si.getMasterLevel()); // for now, maybe always?
    ...
    public static void loadMobSkillsFromWz() {
        String wzDir = ServerConstants.WZ_DIR + "/Skill.wz/MobSkill.wz";
        File topFile = new File(wzDir);
        for (File firs : topFile.listFiles()) {
            File[] files = firs.listFiles();
            Arrays.stream(files).parallel().forEach(file -> {
